...
void viewExam(Student me)
{
    ...
    int sem;
    cin >> sem;
    cout << "id, course, teacher, exam schedule\n";
    while (me.my_course->course->year != year && me.my_course->course->semester_id != sem) me.my_course = me.my_course->next;
    for (MyCourse* cur = me.my_course; cur && cur->course->semester_id == sem; cur = cur->next){
        cout << cur->course->course.course_id << "  " << cur->course->course.course_name << "  " << cur->course->course.teacher_name << "  ";
        for (int i = 0; i < 2; i++){
            switch (cur->course->course.exam_session[i].day_of_the_week){
                case 1:
                    cout << "Sun";
                    break;
                case 2:
            ...
            cout << "-" << cur->course->course.exam_session[i].session_no << "  ";
        }
        cout << endl;
    }
}
